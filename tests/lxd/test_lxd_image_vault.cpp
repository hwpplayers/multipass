...
TEST_F(LXDImageVault, throws_with_invalid_alias)
{
    ...
    mp::Query query{"", "xenial", false, "release", mp::Query::Type::Alias};

    mp::LXDVMImageVault image_vault{hosts, mock_network_access_manager.get(), base_url};

    EXPECT_THROW(image_vault.fetch_image(mp::FetchType::ImageOnly, query, stub_prepare, stub_monitor),
                 std::runtime_error);
}
...
TEST_F(LXDImageVault, throws_with_invalid_remote)
{
    ...
    mp::Query query{"", "foo", false, "bar", mp::Query::Type::Alias};
